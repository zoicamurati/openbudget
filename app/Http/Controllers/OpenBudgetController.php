...
//                ->where('date_executed', 'like', '2023%')
    ...
    public function test(){
        ...
        MAX(inst) as name,
        inst_code,
        CAST(SUM(transact_value) AS INT) as value
    ')
        ...
        foreach ($groupedParents as $groupCode => $groupedParent) {
            ...
            $parentData = [
                ...
                'children' => $children,
                'name' => array_key_exists($groupCode, $category) ? $category[$groupCode] : 'Te tjera'
            ];
        ...
        return view('open_budget_test')->with(['data' => $total_data, 'category'=>$category,'institutions'=>$institutions]);
    }
}
